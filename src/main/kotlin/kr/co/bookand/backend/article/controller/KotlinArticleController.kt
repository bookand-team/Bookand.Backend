...
class KotlinArticleController(
    ...
    @PostMapping("")
    fun createArticle(
        @RequestBody articleRequest: KotlinArticleRequest
    ): ResponseEntity<KotlinArticleIdResponse> {
    ...
    ): ResponseEntity<KotlinArticleDetailResponse> {
        val account = accountService.getCurrentAccount()
 ...
 초기에는 cursorId를 0 넣으시거나 요청 안하시면 됩니다."""
    ...
    fun removeArticle(
        ...
        return ResponseEntity.ok(KotlinMessageResponse(message = "아티클 삭제 완료.", statusCode = 200))
    }

    @ApiOperation(value = "선택된 아티클 삭제")
    @DeleteMapping("/list")
    fun deleteArticleList(
        @RequestBody list: KotlinArticleListRequest
    ): ResponseEntity<KotlinMessageResponse> {
        val account = accountService.getCurrentAccount()
        articleService.removeArticleList(account, list)
        return ResponseEntity.ok(KotlinMessageResponse(message = "선택된 서점 삭제 완료.", statusCode = 200))
    }
